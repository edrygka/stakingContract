...
describe("Staking", () => {
  ...
  describe("stake", () => {
    ...
    it("should stake Alice -> stake Bob -> distribute -> unstake successfully", async () => {
      ...
      expect(await token.balanceOf(alice.address)).to.equal(
        aliceTotalBalance - stakeAlice
      );
      expect(await token.balanceOf(bob.address)).to.equal(
        bobTotalBalance - stakeBob
      );
      ...
      expect(await token.balanceOf(alice.address)).to.equal(
        aliceTotalBalance + (stakeAlice * reward) / (stakeBob + stakeAlice)
      );
      expect(await token.balanceOf(bob.address)).to.equal(
        bobTotalBalance + (stakeBob * reward) / (stakeBob + stakeAlice)
      );
    });
  });
  ...
  describe("unstake", () => {
    ...
    it("should unstake tokens successfully", async () => {
      await expect(staking.unstake()).to.be.revertedWith("Staking: NOT EXIST");
    });
  });
});
